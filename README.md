...
### 1. Cài đặt Python và các thư viện hỗ trợ
Để chạy giao diện phần mềm, máy tính cần cài đặt Python 3 và thư viện xử lý ảnh.

*   **Trên Linux (Ubuntu/Debian)**:
    Mở ứng dụng Terminal lên và chạy lệnh sau:
    ```bash
    sudo apt-get update
    sudo apt-get install python3 python3-pip python3-pil python3-pil.imagetk qrencode zbar-tools -y
    pip3 install requests pillow
    ```

*   **Trên Windows / macOS**:
    1. Tải và cài đặt [Python 3](https://www.python.org/downloads/). (Nhớ tích chọn "Add Python to PATH" khi cài đặt).
    2. Mở Command Prompt (Windows) hoặc Terminal (macOS) và chạy:
       ```bash
